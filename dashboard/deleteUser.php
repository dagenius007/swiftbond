...
if ($_GET['rdr'] == md5(md5("3e7a5abf659d1c90b2760d830ca67ba1")) && isset($_GET['du'])) {
	...
	if ($_GET['du'] == "dpay1") {
		...
		$sql = "DELETE FROM `users` WHERE `email` = '" . $_SESSION['dpay3'] . "'";
		if ($conn->query($sql) === TRUE) {
			$conn->close();
			require('connectdb.ext');
			$sql = "UPDATE `users` SET `dpay3v`='' WHERE `email` = '" . $_SESSION['email'] . "'";
			if ($conn->query($sql) === TRUE) {
				$conn->close();
				require('connectdb.ext');
				$sql = "UPDATE `users` SET `dpay3`='' WHERE `email` = '" . $_SESSION['email'] . "'";
				if ($conn->query($sql) === TRUE) {
					$conn->close();
				}
			}
		...
		$sql = "DELETE FROM `users` WHERE `email` = '" . $_SESSION['dpay4'] . "'";
		if ($conn->query($sql) === TRUE) {
			$conn->close();
			require('connectdb.ext');
			$sql = "UPDATE `users` SET `dpay4v`='' WHERE `email` = '" . $_SESSION['email'] . "'";
			if ($conn->query($sql) === TRUE) {
				$conn->close();
				require('connectdb.ext');
				$sql = "UPDATE `users` SET `dpay4`='' WHERE `email` = '" . $_SESSION['email'] . "'";
				if ($conn->query($sql) === TRUE) {
					$conn->close();
				}
			}
